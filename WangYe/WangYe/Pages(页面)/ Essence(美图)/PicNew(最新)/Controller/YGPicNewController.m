...
@implementation YGPicNewController
...
- (void)configNetManager
{
    ...
    [self.collectionView addHeaderRefresh:^{
        [NetManager GETPicNewItem:0 completionHandler:^(YGPicAllItem *picNewItem, NSError *error) {
            [weakSelf.collectionView endHeaderRefresh];
            if (error) {
                [weakSelf.view showMessage:@"网络有误..."];
            } else {
                [weakSelf.picHotArr removeAllObjects];
                [weakSelf.picHotArr addObjectsFromArray:picNewItem.res.data];
                
                //清空所有
                [weakSelf.picArr removeAllObjects];
                //便利出图片数组
                for (YGPicAllResDataItem *dataItem in picNewItem.res.data) {
                    [weakSelf.picArr addObject:dataItem.url];
                }
                
                [weakSelf.collectionView reloadData];
                if (picNewItem.res.data.count < 60) {
    ...
    [self.collectionView addFooterRefresh:^{
        [NetManager GETPicNewItem:weakSelf.pageCount + 60 completionHandler:^(YGPicAllItem *picNewItem, NSError *error) {
...
- (NSMutableArray<YGPicAllResDataItem *> *)picHotArr {
    if(_picHotArr == nil) {
